...
function BlogList() {
  ...
  const notify = async (id: string, title: string) => {
    ...
    const r = await supabase.functions.invoke("notify-new-blog", { body: { post_id: id } });
    toast.dismiss(t);
              ...
              <tr key={p.id} className="hover:bg-secondary/30">
                ...
                <td className="p-3 text-right">
                  <div className="inline-flex gap-1">
                    ...
                    {p.status === "published" && <button onClick={() => notify(p.id, p.title)} className="p-2 rounded-lg hover:bg-secondary text-[color:var(--gold)]" title="Email subscribers"><Mail className="h-4 w-4" /></button>}
